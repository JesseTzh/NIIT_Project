...
class AfterSale extends Model {
    ...
    {    //每页显示12条数据
        ...
        $num
            = session('employee_character_num');
        $depart_num = session('employee_department_num');
        $kz = $request->param
        ('condition');
        //dump($depart_num);
        if ($num == 0)
            return $this->where(json_decode($kz))->paginate
            ($itemNum);
        else if ($depart_num >= 1 && $depart_num <= 9)
            return $this->where('after_sale_department',
                $depart_num)->paginate($itemNum);
        else
            return $this->where('after_sale_empolyee_id', $condition)->paginate($itemNum);
    }
